...
def _parse_dhm(dhm: str) -> Tuple[int, int]:
  ...
  if not re.fullmatch(r"(\d+d)?(\d+h)?(\d+m)?", dhm):
    raise ValueError(
      "Invalid format. Make sure to use the format: 1d2h3m, and each unit only exists once."
    )
...
def compute_gap_days_and_end_slot(slot: str, duration: str) -> Tuple[int, str]:
  ...
  slots = get_trading_slots(
    begin="0930", end="1500", freq_in_minute=1, bar_on_the_right=True
  )
...
def get_tcalendar_df(n_next: int = 1) -> pl.DataFrame:
  ...
  df = df.select(pl.col("date"), pl.col("date").shift(-n_next).alias("next"))
  return df
...
def adjust_tcalendar_slot_df(
  duration: str, start_slot: str | list[str] = "0930"
) -> pl.DataFrame:
  ...
  for _slot in start_slot:
    ...
    start_duration = pl.duration(
      hours=int(_slot[:2]), minutes=int(_slot[2:]), time_unit="ns"
    )
    end_duration = pl.duration(
      hours=int(end_slot[:2]), minutes=int(end_slot[2:]), time_unit="ns"
    )
    df = df.select(
      pl.col("date")
